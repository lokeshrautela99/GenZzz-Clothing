...
const slideShow = () => {
  const arr = [
    "./images/ProductData/Women/women's Slider/women1.avif",
    "./images/ProductData/Women/women's Slider/women2.avif",
    "./images/ProductData/Women/women's Slider/women3.PNG",
  ];
...
let pages = Math.ceil(womenDataArr.length / product_per_page);

let womenData = document.getElementById("dataDiv");
function renderWomenData(data) {
  womenData.innerHTML = null;
...
document.getElementById("sortHL").addEventListener("click", () => {
  let sort = womenDataArr.sort((a, b) => b.price - a.price);
  renderWomenData(sort);
});

document.getElementById("sortLH").addEventListener("click", () => {
  let sort = womenDataArr.sort((a, b) => a.price - b.price);
  renderWomenData(sort);
});
...
filter.addEventListener("change", () => {
  if (filter.value == 0 || filter.value == null) {
    ...
    let filterData = womenDataArr.filter((ele) => {
      return ele.category === filter.value;
    });
    renderWomenData(filterData);
  }
});
...
function searchFun() {
  ...
  let searchedWomenData = womenDataArr.filter(function (ele) {
    return (
  ...
  if (searchedWomenData.length == 0 || searchVal == "") {
      document.getElementById("searchAlert").style.display = "block";
    setTimeout(() => {
      document.getElementById("searchAlert").style.display = "none";
    }, 1000);
  } else {
    renderWomenData(searchedWomenData);
  }
}
...
function renderPagination(pages) {
  for (let i = 1; i <= pages; i++) {
    ...
    btn.addEventListener("click", function () {
      ...
      paginatedTable(womenDataArr, currentPageState, product_per_page);
    });
